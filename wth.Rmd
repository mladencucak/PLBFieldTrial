...
dfday %>% 
  ...
  geom_line(
    aes(
      x = date,
      y = rhum,
      colour = "Average relative humidity (%)"
    ),
    size = 0.25
  ) +
  geom_line(
  ...
  scale_x_date(date_labels = "%b", date_breaks = "1 month",expand = c(.03, .03))+
  ...
  # annotate(geom = "text",  y = 55, x = dfday$disout,size = 8, colour = "red")
  geom_text(aes(label= symptoms, y = 39, x = disout),size = 2.5)+
  geom_text(aes(label= "Last assesment", y = 39, x = disend),size = 2.5)+
